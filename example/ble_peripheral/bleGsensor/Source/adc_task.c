...
adc_Cfg_t adc_cfg =
{
	.channel = ADC_BIT(ADC_CH3P_P20)|ADC_BIT(ADC_CH2P_P14)|ADC_BIT(ADC_CH1N_P11),
...
void adcTaskInit( uint8 task_id )
{
    adcDemo_TaskID = task_id;
    adcMeasureTask();
}

uint16 adcTaskProcessEvent( uint8 task_id, uint16 events )
{
...
static void adcMeasureTask( void )
{
    ...
    {
      osal_start_timerEx(adcDemo_TaskID, adcMeasureTask_EVT, 60*1000);
    }
}
